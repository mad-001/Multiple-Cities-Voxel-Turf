...
static DWORD WINAPI LaunchBridgeThread(LPVOID) {
    ...
    _snprintf(bridge, sizeof(bridge), "%s\\mods\\TakaroConnector\\bridge\\bridge.js", gameDir);
    if (GetFileAttributesA(bridge) == INVALID_FILE_ATTRIBUTES) return 0;
...
BOOL APIENTRY DllMain(HMODULE, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        if (g_bridgeProc) { TerminateProcess(g_bridgeProc, 0); CloseHandle(g_bridgeProc); g_bridgeProc = NULL; }
